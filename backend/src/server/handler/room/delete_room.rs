...
use crate::database::{Deletable, Selectable};
use crate::models::group::Group;
use crate::models::room::Room;
use crate::models::user::User;
use crate::server::error::{CustomError, GROUP_SELECT_ERROR, ROOM_DELETE_ERROR, ROOM_NOT_EXIST_ERROR, ROOM_NOT_THE_OWNER_ERROR};
use crate::server::handler::Handler;

pub struct DeleteRoomHandler;
pub struct DeleteRoomHandlerResources {
    database_room: Room,
    database_group: Option<Group>
}

impl Handler for DeleteRoomHandler {
    type Body = ();
    type Path = (String,);
    type Query = ();
    type Resources = DeleteRoomHandlerResources;
    ...
    async fn save_resources(&self, _req: HttpRequest, resources: &mut Self::Resources, _user: Option<User>) -> Result<(), CustomError> {
        match resources.database_room.delete().await {
            Ok(_) => Ok(()),
            Err(err) => Err(ROOM_DELETE_ERROR.with_details(err.to_string()))
        }
    }
