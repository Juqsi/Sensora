use actix_web::{HttpRequest, HttpResponse, Responder};
use crate::database::{Deletable, Selectable};
use crate::models::group::Group;
use crate::models::user::User;
use crate::server::error::{CustomError, GROUP_DELETE_ERROR, GROUP_NOT_EXIST_ERROR, NOT_A_MEMBER_ERROR};
use crate::server::handler::Handler;

pub struct DeleteGroupHandler;

impl Handler for DeleteGroupHandler {
    type Body = ();
    type Path = (String,);
    type Query = ();
    type Resources = Group;

    fn new() -> Self {
        DeleteGroupHandler
    }

    async fn load_resources(&self, req: HttpRequest, _content: Self::Body, _user: Option<User>) -> Result<Self::Resources, CustomError> {
        let group_id = req.match_info().get("groupId");
        
        match Group::select(group_id.unwrap().to_string()).await {
            Ok(group) => Ok(group),
            Err(err) => Err(GROUP_NOT_EXIST_ERROR.with_details(err.to_string()))
        }
    }

    fn authorize(&self, _req: HttpRequest, resources: &mut Self::Resources, user: Option<User>) -> Result<(), CustomError> {
        if resources.is_member(user.unwrap()) {
            return Ok(())
        }
        Err(NOT_A_MEMBER_ERROR)
    }

    async fn save_resources(&self, _req: HttpRequest, resources: &mut Self::Resources, _user: Option<User>) -> Result<(), CustomError> {
        match resources.delete().await {
            Ok(_) => Ok(()),
            Err(err) => Err(GROUP_DELETE_ERROR.with_details(err.to_string()))
        }
    }

    fn response(self, _req: HttpRequest, _resources: Self::Resources) -> impl Responder
    where
        Self: Sized,
    {
        HttpResponse::NoContent().finish()
    }
}
